...
public final class OpCommands {
    ...
    private static int op(CommandContext<CommandSourceStack> ctx, NeoForgeServer server, HybridIdentity identity)
    ...
    private static int deop(CommandContext<CommandSourceStack> ctx, NeoForgeServer server, HybridIdentity identity)
            throws CommandSyntaxException {
        CommandSourceStack source = ctx.getSource();
        GameProfile profile = new GameProfile(identity.minecraftUUID(), identity.username());
