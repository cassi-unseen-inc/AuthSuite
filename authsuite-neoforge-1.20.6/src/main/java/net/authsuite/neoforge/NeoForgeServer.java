...
import net.authsuite.neoforge.perm.NeoPermissionService;
import net.authsuite.neoforge.playerdata.PlayerDataRouter;
...
public final class NeoForgeServer {
    ...
    private static volatile NeoForgeServer instance;
    ...
    private NeoForgeServer(AuthSuiteLogger log) {
        this.log = log;
    ...
    public static void init(IEventBus modEventBus, ModContainer modContainer, AuthSuiteLogger log) {
        synchronized (LOCK) {
            if (instance == null) {
                instance = new NeoForgeServer(log);
            }
        }
        NeoForge.EVENT_BUS.addListener(instance::onServerAboutToStart);
        NeoForge.EVENT_BUS.addListener(instance::onServerStarting);
        NeoForge.EVENT_BUS.addListener(instance::onRegisterCommands);
        NeoForge.EVENT_BUS.addListener(instance::onPlayerLoggedIn);
        NeoForge.EVENT_BUS.addListener(instance::onPlayerLoggedOut);
        NeoForge.EVENT_BUS.addListener(instance::onServerStopping);
        NeoForge.EVENT_BUS.addListener(instance::onServerStopped);
        NeoForgeNetwork.registerClientHandlers(modEventBus);
    }

    public static NeoForgeServer get() {
        return instance;
    }
    ...
    private void onServerAboutToStart(ServerAboutToStartEvent event) {
        ...
        this.api = new NeoForgeAuthSuiteAPI(identityRegistry, providerManager, permissionService, opsRouter, log);
    ...
    public NeoPermissionService permissions() {
    ...
    public NeoForgeAuthSuiteAPI api() {
        return api;
    }
