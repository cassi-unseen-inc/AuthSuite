...
public final class AuthSuiteCommands {
    ...
    private static int whoami(CommandContext<CommandSourceStack> ctx, FabricServer server) throws CommandSyntaxException {
        if (!(ctx.getSource().getEntity() instanceof ServerPlayer player)) {
            ctx.getSource().sendFailure(new TextComponent("Only players can use /authsuite whoami"));
            return 0;
        }
        ...
        HybridIdentity identity = registry.byUuid(player.getUUID())
                ...
                .orElse(null);
        if (identity == null) {
            ctx.getSource().sendFailure(new TextComponent("No AuthSuite identity registered for your session"));
            return 0;
        }
        ctx.getSource().sendSuccess(new TextComponent(
