...
public final class OpCommandInterceptor {
     ...
     */
    public static boolean tryHandle(CommandSource source, String raw) {
        ...
        java.util.List<String> tokens = java.util.Arrays.asList(parts);
        if (tokens.size() < 3) {
            return false;
        }
        ...
        if (op) {
            ...
            source.sendSuccess(new StringTextComponent("Made " + playerName + " an operator in provider "
                    + identity.providerId()), true);
        } else {
            server.opsRouter().deopFor(identity.providerId(), profile);
            source.sendSuccess(new StringTextComponent("Removed " + playerName + " from operators in provider "
                    + identity.providerId()), true);
        }
        return true;
    }

    /** Called from the CommandDispatcherMixin before vanilla execution (1.18.2 signature). */
    public static boolean preDispatch(CommandSource source, String command) {
