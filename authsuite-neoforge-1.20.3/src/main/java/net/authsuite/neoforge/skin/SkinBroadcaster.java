...
public final class SkinBroadcaster {
     ...
     * <p>
     * Deviation from the 1.20.4 reference: 1.20.2/1.20.3 ship pre-payload-registry
     * {@code SimpleChannel} packets (not {@code CustomPacketPayload}), so the
     * {@code ServerGamePacketListenerImpl#send(CustomPacketPayload)} extension does
     * not exist here; the sends go through {@link NeoForgeNetwork#sendToPlayer}.
     */
    public void broadcast(ServerPlayer player, HybridIdentity identity) {
        ...
        for (ServerPlayer other : player.server.getPlayerList().getPlayers()) {
            if (other != player && !other.getUUID().equals(player.getUUID())) {
                NeoForgeNetwork.sendToPlayer(other, PlayerSkinDirectivePayload.of(directive));
            }
        }
        for (Map.Entry<UUID, SkinDirective> entry : directives.entrySet()) {
            if (!entry.getKey().equals(player.getUUID())) {
                NeoForgeNetwork.sendToPlayer(player, PlayerSkinDirectivePayload.of(entry.getValue()));
            }
        }
        NeoForgeNetwork.sendToPlayer(player, PlayerSkinDirectivePayload.of(directive));
