package net.authsuite.forge.login;
...
/**
 * Builds a canonical {@link GameProfile} from a provider-validated
 * {@link AuthenticatedProfile}.
 * <p>
...
public final class LoginProfileBuilder {
    ...
    public GameProfile buildProfileResult(AuthResolver.Resolution resolution, String username, String serverId) {
    ...
    private Map<String, String> skinMetadata(AuthenticatedProfile profile) {
        ...
        m.put("skin", skinUrl(profile));
        m.put("cape", capeUrl(profile));
        m.put("model", modelType(profile));
        return m;
    }

    private SkinResource skinResource(AuthenticatedProfile profile, String key) {
        String url = skinUrl(profile);
        return url == null || url.trim().isEmpty() ? null : new SkinResource(url);
    }
    ...
    private String extractTextureUrl(AuthenticatedProfile profile, String kind) {
        ...
        if (textures == null || textures.trim().isEmpty()) {
            return null;
        }
        try {
