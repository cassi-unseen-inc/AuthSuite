...
public final class OpCommands {
    ...
    private static int op(CommandContext<CommandSourceStack> ctx, ForgeServer server, ServerPlayer target)
    ...
    private static int deop(CommandContext<CommandSourceStack> ctx, ForgeServer server, ServerPlayer target)
            throws CommandSyntaxException {
        CommandSourceStack source = ctx.getSource();
        HybridIdentity identity = identityFor(server, target);
        if (identity == null) {
    ...
    private static HybridIdentity identityFor(ForgeServer server, ServerPlayer target) {
