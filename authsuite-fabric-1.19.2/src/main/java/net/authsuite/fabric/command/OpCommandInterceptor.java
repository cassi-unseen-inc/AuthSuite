...
public final class OpCommandInterceptor {
    ...
    public static boolean tryHandle(CommandSourceStack source, String raw) {
        ...
        if (op) {
            ...
            source.sendSuccess(Component.literal("Made " + playerName + " an operator in provider "
                    + identity.providerId()), true);
        } else {
            server.opsRouter().deopFor(identity.providerId(), profile);
            source.sendSuccess(Component.literal("Removed " + playerName + " from operators in provider "
                    + identity.providerId()), true);
        }
        return true;
    }
    ...
    public static boolean preDispatch(ParseResults<CommandSourceStack> results, String command) {
