package net.authsuite.forge.perm;
...
/**
 * NeoForge implementation of {@link PermissionService}. Permission evaluation is
 * based exclusively on the server's authenticated identity registry, never on
 * client-supplied identity data (spec §11).
 */
public final class ForgePermissionService implements PermissionService {
    ...
    public ForgePermissionService(IdentityRegistry identityRegistry, ProviderManager providerManager,
    ...
    private OpsRouter router() {
        return net.authsuite.forge.ForgeServer.get() != null
                ? net.authsuite.forge.ForgeServer.get().opsRouter()
                : null;
    }
