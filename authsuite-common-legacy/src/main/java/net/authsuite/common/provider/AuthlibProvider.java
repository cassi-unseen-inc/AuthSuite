...
public final class AuthlibProvider implements AuthProvider {
    ...
    public CompletableFuture<AuthenticatedProfile> authenticate(AuthAttempt attempt) {
        ...
        if (config.checkUrl() == null || config.checkUrl().trim().isEmpty()) {
            return failedFuture(new ProviderFailureException(
    ...
    private AuthenticatedProfile parseResponse(AuthAttempt attempt, String body) {
        ...
        return AuthenticatedProfile.withTtl(providerId, accountId, playerName, profileUuid,
                Collections.unmodifiableMap(new HashMap<>(textures)), 300_000);
    }
    ...
    private AuthenticatedProfile unwrap(Throwable ex, AuthAttempt attempt) {
        ...
        if (ex instanceof ProviderFailureException) {
            throw (ProviderFailureException) ex;
        }
        Throwable cause = ex;
        if (cause instanceof ProviderHttpClient.HttpException) {
            ProviderHttpClient.HttpException httpEx = (ProviderHttpClient.HttpException) cause;
    ...
    public CompletableFuture<ResolverResult> resolvePlayer(String username) {
        if (config.profilesUrl() == null || config.profilesUrl().trim().isEmpty()) {
            return CompletableFuture.completedFuture(null);
        }
        Map<String, String> params = new HashMap<>();
        params.put("username", username);
        URI url = ProviderHttpClient.joinQuery(URI.create(config.profilesUrl()), params);
        return http.get(url, 10_000, 1_048_576)
